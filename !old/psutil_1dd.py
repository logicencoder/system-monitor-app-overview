...
class CPUMonitor(BaseMonitor):
    """CPU usage and statistics monitor with proper frequency display."""
    
    def on_mount(self) -> None:
        self.set_interval(0.5, self.refresh)
    
    def get_freq_mhz(self, hz: float) -> float:
        """
        Convert CPU frequency from Hz to MHz.
        Args:
            hz: Frequency in Hz
        Returns:
            Frequency in MHz
        """
        return hz / 1000000  # Convert Hz to MHz
    ...
    def render(self) -> Panel:
        ...
        current_mhz = self.get_freq_mhz(freq.current * 1000000)  # psutil returns MHz, convert to Hz then back
        min_mhz = self.get_freq_mhz(freq.min * 1000000)
        max_mhz = self.get_freq_mhz(freq.max * 1000000)

        table.add_row(
            "CPU Total",
            create_bar(total),
            self.format_detail(
                f"Freq: {current_mhz:.0f}MHz",
                f"Tasks: {format_bytes(ctx.ctx_switches)}/s"
            )
        )
        ...
        table.add_row(
            ...
            self.format_detail(
                f"User: {times.user:4.1f}%",
                f"Sys: {times.system:4.1f}%",
                f"Idle: {times.idle:4.1f}%"
            )
        )

        # Show per-core usage and frequencies
        for i, percent in enumerate(cpu_percent):
            freq_info = f"Min: {min_mhz:.0f}MHz Max: {max_mhz:.0f}MHz" if i == 0 else ""
...
class EnhancedStorageMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        return Panel(table, title="Enhanced Storage Monitor", border_style="magenta")
