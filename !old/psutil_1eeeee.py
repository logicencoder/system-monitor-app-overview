...
MONITOR_INTERVALS = {
    'cpu': 0.5,      # CPU monitor refresh
    'memory': 0.5,   # Memory monitor refresh
    'network': 0.5,  # Network monitor refresh
    'disk': 0.5,     # Disk monitor refresh
    'gpu': 0.5,      # GPU monitor refresh
    'sensors': 0.5,  # Sensors monitor refresh
...
class NetworkMonitor(BaseMonitor):
    ...
    def __init__(self):
        ...
        self.history = {
            'bytes_sent': deque(maxlen=10),
            'bytes_recv': deque(maxlen=10),
    ...
    def _get_network_metrics(self) -> Dict[str, Any]:
        """Calculate network metrics and rates."""
        try:
            now = time.time()
        ...
        try:
            ...
            if 'rates' in metrics:
                ...
                if 'bytes_recv' in rates:
                    table.add_row(
                        "Download",
                        f"↓ {format_bytes(rates['bytes_recv'])}/s",
                ...
                if 'bytes_sent' in rates:
                    table.add_row(
                        "Upload",
                        f"↑ {format_bytes(rates['bytes_sent'])}/s",
            ...
            if metrics.get('error_in', 0) > 0 or metrics.get('error_out', 0) > 0:
                table.add_row(
                    "Errors",
...
class GPUMonitor(BaseMonitor):
    """GPU monitor optimized for NVIDIA cards."""
    ...
    def _get_gpu_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'name': 'NVIDIA GeForce GTX 750 Ti',  # Known GPU model
                'usage': 0,
                'temp': 0,
                'memory_used': 0,
                'memory_total': 2048,
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            for temp in sensor_data['temperatures']:
                ...
                table.add_row(
                    temp['name'][:12],
                    create_progress_bar(min(100, temp['current']), color=status_color),
                    " | ".join(details)
                )
            
            # Show power readings if available
            if sensor_data['power']:
                if sensor_data['temperatures']:  # Add separator if we had temperatures
                    table.add_row("", "", "")
                table.add_row("--- Power ---", "", "")
            ...
            # Show fan speeds
            if sensor_data['fans']:
                if sensor_data['temperatures'] or sensor_data['power']:  # Add separator if needed
                    table.add_row("", "", "")
                table.add_row("--- Fans ---", "", "")
