...
# Add SelfMonitor class to monitor app's own resource usage
...
class SelfMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            # Changed padding from (0,0) to (0,2) for better spacing
            ...
            table.add_row(
                "CPU Overhead",
                create_progress_bar(metrics['cpu_overhead']),
                f"{metrics['cpu_overhead']:.2f}% of system | Avg: {metrics['avg_cpu']:.1f}%"
            )
            
            # Memory usage with overhead
            table.add_row(
                "Memory",
...
# Fixed this function that was undefined
...
# Show fan speeds
            ...
            if battery_info['available']:
                ...
                table.add_row(
                    "Charge",
                    create_progress_bar(battery_info['percent'], color=status_color),
...
class ProcessMonitor(BaseMonitor):
    ...
    def __init__(self):
        try:
            ...
            self.processes_limit = config['monitors']['process']['limit']  # Get limit from config (20)
            ...
            self.sort_by = 'cpu'
            self.processes_limit = 20  # Default to 20 if config fails
            self.last_process_check = 0
            self.process_cache = []
            self.process_cache_ttl = 2
    
    def on_key(self, event) -> None:
        """Handle key presses for sorting."""
        try:
            if event.key == "c":
                self.sort_by = 'cpu'
                logger.info("Sorting processes by CPU usage")
                self.last_process_check = 0  # Force refresh
            elif event.key == "m":
                self.sort_by = 'memory'
                logger.info("Sorting processes by memory usage")
                self.last_process_check = 0  # Force refresh
        except Exception as e:
            logger.error(f"Error in process monitor key handler: {e}")
    ...
    def _get_processes(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Update cache - limit to configured number of processes (20)
    ...
    def render(self) -> Panel:
        """Render process information panel showing top 20 processes."""
        try:
            # THIS IS THE ACTUAL FIX - using rich.table with minimal settings
            from rich.console import Console
            from rich.table import Table as RichTable
            
            # Create a rich table with minimal settings for a compact display
            table = RichTable(
                box=None,
                expand=True,
                ...
                show_lines=False,
                show_edge=False,
                row_styles=["", ""],  # Alternating row styles but without additional height
            )
            
            # Add columns with proper spacing
            table.add_column("PID", style="cyan", width=7, no_wrap=True)
            table.add_column("Name", style="bright_blue", width=25, no_wrap=True)
            table.add_column("CPU%", justify="right", width=7, no_wrap=True)
            table.add_column("Memory", justify="right", width=10, no_wrap=True)
            table.add_column("Status", width=10, no_wrap=True)
            
            # Get process information - get all 20 processes as configured
            processes = self._get_processes()
            
            # Add sorting information to the title
            sort_info = "CPU" if self.sort_by == 'cpu' else "Memory" if self.sort_by == 'memory' else "Time"
            
            # Show processes - show all 20 processes
            if processes:
                for proc in processes:
                    ...
                    # Add process row with minimal height
                    table.add_row(
                        str(proc.get('pid', '')),
                        proc.get('name', '')[:25],
                        cpu_text,
...
class SystemMonitorApp(App):
    """Main system monitoring application without key bindings."""
