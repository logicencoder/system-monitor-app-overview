...
DEFAULT_CONFIG = {
    'monitors': {
        ...
        'self': {'enabled': True, 'interval': 1.0}  # Add self monitor config
    },
    'ui': {
        'theme': 'dark',
        'left_column': ['self', 'cpu', 'gpu', 'process', 'services'],  # Add 'self' at the top of left column
        'right_column': ['memory', 'disk', 'network', 'sensors', 'battery', 'firewall'],  # Move 'battery' after 'sensors'
        'cores_per_line': 4
    },
    'alerts': {
        'enabled': True,
...
class ProcessMonitor(BaseMonitor):
    ...
    def _get_processes(self) -> List[Dict[str, Any]]:
        """Get sorted process information with enhanced Python process details."""
        try:
            current_time = time.time()
            ...
            for proc in psutil.process_iter(['pid', 'name', 'username', 'cpu_percent', 'memory_percent', 'create_time', 'status']):
                try:
                    ...
                    try:
                        ...
                        # Enhanced Python process identification
                        if proc_info['name'] in ['python', 'python3', 'py', 'python.exe', 'python3.exe']:
                            # Look for the script name in command line
                            if cmdline and len(cmdline) > 1:
                                script_path = cmdline[1]
                                script_name = os.path.basename(script_path)
                                # Add script name to process name for better identification
                                proc_info['display_name'] = f"{proc_info['name']}:{script_name}"
                            else:
                                proc_info['display_name'] = proc_info['name']
                        else:
                            proc_info['display_name'] = proc_info['name']
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            table.add_column("Name", style="bright_blue", width=25)  # Increased width for longer names
            ...
            if processes:
                for proc in processes:
                    ...
                    # Add process row with display_name instead of name
                    table.add_row(
                        str(proc.get('pid', '')),
                        proc.get('display_name', '')[:25],  # Use display_name with longer limit
                        cpu_text,
                        mem_text,
                        status_text
                    )
...
class SelfMonitor(BaseMonitor):
    ...
    def __init__(self):
        try:
            ...
            logger.debug("Self monitor initialized")
        except Exception as e:
    ...
    def _get_self_metrics(self) -> Dict[str, Any]:
        """Get resource usage metrics for this application."""
        try:
            if not self.process:
                return {}
    ...
    def render(self) -> Panel:
        """Render self monitoring information panel."""
        try:
            table = Table(box=None, expand=True, padding=(0,0))
            table.add_column("Monitor", style="cyan", width=12)
            table.add_column("Usage", ratio=2)
            table.add_column("Details", style="bright_blue")
            ...
            if not metrics:
                return Panel(Text("Self monitoring unavailable", style="yellow"), title="Monitor Usage", border_style="green")
            
            # CPU usage
            table.add_row(
                "CPU",
                create_progress_bar(metrics['cpu_percent']),
                f"Avg: {metrics['avg_cpu']:.1f}%"
            )
            ...
            table.add_row(
                "Threads",
                f"{metrics['threads']} threads",
                ""
            )
            
            # I/O if available
            if metrics.get('read_bytes', 0) > 0 or metrics.get('write_bytes', 0) > 0:
                table.add_row(
                    "I/O",
            ...
            return Panel(table, title="Monitor Usage", border_style="green")
        except Exception as e:
            self.handle_error(e, "render")
            return Panel(Text(f"Self Monitor Error: {str(e)}", style="red"), title="Monitor Usage", border_style="green")
...
class SystemMonitorApp(App):
    ...
    BINDINGS = [
        ...
        Binding("u", "toggle_self", "Monitor Usage"),  # Added binding for self monitor
    ]
    ...
    def __init__(self):
        try:
            ...
            try:
                self.monitors['self'] = SelfMonitor()  # Add self monitor first
    ...
    def action_toggle_self(self) -> None:  # Added toggle for self monitor
        self._toggle_monitor('self')
    ...
    def compose(self) -> ComposeResult:
        try:
            # Header info with app's own resource usage
            uptime = datetime.now() - self.start_time
            uptime_str = f"{uptime.seconds // 3600}h {(uptime.seconds % 3600) // 60}m {uptime.seconds % 60}s"
            
            # Try to get current CPU and memory usage for header
            try:
                process = psutil.Process(os.getpid())
                cpu_usage = process.cpu_percent(interval=0.1)
                mem_usage = process.memory_percent()
                resource_info = f"App: CPU {cpu_usage:.1f}% | Mem {mem_usage:.1f}%"
            except:
                resource_info = ""
            ...
            header_text = [
                ...
                resource_info if resource_info else "",
                f"Uptime: {uptime_str}"
            ]
            # Filter out empty strings
            header_text = [text for text in header_text if text]
    ...
    def _update_header(self) -> None:
        ...
        try:
            ...
            # Get the header and use set_text method instead of update
