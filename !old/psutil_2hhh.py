...
# Function to get terminal width for true responsive design
def get_terminal_width():
    """Get current terminal width for responsive calculations"""
    try:
        # First try to get terminal size from os module
    ...
    try:
        # Try through environment variables
...
# FIXED PROGRESS BAR FUNCTION TO REMOVE THE THREE DOTS
def create_responsive_progress_bar(percentage: float, available_width: int = None,
                                  show_percentage: bool = True, custom_color: str = None,
                                  is_core_monitor: bool = False) -> Text:
    """Create a truly responsive progress bar that adapts to container width."""
    try:
        # Ensure percentage is within bounds
        percentage = max(0, min(100, percentage))
        
        # Use a default width if none is provided
        if available_width is None or available_width <= 0:
            available_width = 40  # Safe default
            
        # Reserve space for percentage if showing it
        effective_width = available_width
        if show_percentage:
            # Reserve about 8 chars for the percentage display
            effective_width = max(10, available_width - 8)
        ...
        # Create strings first, then convert to styled Text objects
        filled_str = '■' * filled
        remainder_str = '·' * remainder
        
        # Create styled Text objects
        filled_text = Text(filled_str, style=color)
        remainder_text = Text(remainder_str, style="bright_black")
        
        # Combine them - no concatenation that might cause truncation
        bar = Text("")
        bar.append(filled_text)
        bar.append(remainder_text)
        
        # Add percentage text if enabled
        if show_percentage:
            percentage_text = Text(f" {percentage:5.1f}%", style=color)
            result = Text("")
            result.append(bar)
            result.append(percentage_text)
            return result
        else:
            return bar
    except Exception as e:
        # Log the error but return something that won't crash
...
class BaseMonitor(Static):
    ...
    def __init__(self):
        try:
            ...
            self.error_count = 0
            self.max_errors = 3
            # Set a default width before trying to get the actual width
            self.console_width = 80  
            try:
                # Only try to get terminal width if possible
                self.console_width = get_terminal_width()
            except Exception:
                # Keep using the default if it fails
                pass
    ...
    def update_dimensions(self):
        """Update dimensions for responsive layout - safely"""
        try:
            new_width = get_terminal_width()
            if new_width > 0:  # Only update if we got a valid width
                self.console_width = new_width
        except Exception as e:
            # Don't log this to avoid spamming the logs
            pass

    def create_monitor_table(self):
        """Create a table with settings that prevent the three dots truncation"""
        table = Table(
            box=None, 
            expand=True, 
            padding=(0,0),
...
class SelfMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            # Update dimensions for responsiveness
            self.update_dimensions()
            
            table = self.create_monitor_table()
            
            # Create responsive columns
            col_width = max(10, self.console_width // 10)
            table.add_column("Resource", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Usage", ratio=1, overflow="hidden")
            table.add_column("Details", style="bright_blue", ratio=1, overflow="hidden")
...
class CPUMonitor(BaseMonitor):
    ...
    def create_core_row(self, start_idx: int, cpu_percent: list) -> list:
        """Create a row of CPU core displays using responsive design."""
        cores_in_row = []
        try:
            # Calculate available width per core 
            term_width = self.console_width
            width_per_core = max(15, (term_width // CORES_PER_LINE) - 10)
            
            for i in range(start_idx, min(start_idx + CORES_PER_LINE, len(cpu_percent))):
                color = self.get_usage_color(cpu_percent[i])
                # Use responsive progress bar with correct width calculation
                core_text = Text(f"Core {i:2d}: ", "cyan") + create_responsive_progress_bar(
                    cpu_percent[i], 
                    width_per_core, 
                    custom_color=color, 
                    is_core_monitor=True
                )
    ...
    def render(self) -> Panel:
        try:
            ...
            # Create metrics header table - responsive
            metrics_table = self.create_monitor_table()
            metrics_table.add_column("Total CPU", justify="left", style="cyan", ratio=1, overflow="hidden")
            metrics_table.add_column("System Info", justify="left", style="cyan", ratio=1, overflow="hidden")
            ...
            # Calculate available width for progress bar
            avail_width = max(20, (self.console_width // 2) - 30)
            
            # Use unified progress bar for total CPU usage
            total_color = self.get_usage_color(total)
            metrics_table.add_row(
                Text("Total: ") + create_responsive_progress_bar(total, avail_width, custom_color=total_color),
            ...
            # Create cores table with dynamic columns - responsive
            cores_table = self.create_monitor_table()
            
            # Add columns based on CORES_PER_LINE
            for i in range(CORES_PER_LINE):
                cores_table.add_column("", ratio=1, overflow="hidden")  # Empty column title
...
class MemoryMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        try:
            ...
            table.add_column("Memory", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Usage", ratio=1, overflow="hidden")
            table.add_column("Details", style="bright_blue", ratio=1, overflow="hidden")
            ...
            # Calculate available width for progress bar
            avail_width = max(20, (self.console_width // 2) - 20)
            
            # RAM Usage - using responsive progress bar
            table.add_row(
                "RAM",
                create_responsive_progress_bar(vm.percent, avail_width),
                f"Used: {format_bytes(vm.used)} / Total: {format_bytes(vm.total)}"
            )
            
            # Cache Usage - using responsive progress bar
            ...
            if hasattr(vm, 'cached'):
                ...
                table.add_row(
                    "Cache",
                    create_responsive_progress_bar(cache_percent, avail_width),
                    cache_text
                )

            # Effective Memory - using responsive progress bar
            if hasattr(vm, 'available') and hasattr(vm, 'cached') and hasattr(vm, 'buffers'):
                ...
                if effective_used >= 0:
                    ...
                    table.add_row(
                        "Effective",
                        create_responsive_progress_bar(effective_percent, avail_width),
                        f"Used: {format_bytes(effective_used)} | Available: {format_bytes(vm.available)}"
                    )
            
            # Swap Usage - using responsive progress bar
            table.add_row(
                "Swap",
                create_responsive_progress_bar(swap.percent, avail_width),
...
class NetworkMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            table.add_column("Network", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Traffic", ratio=1, overflow="hidden") 
            table.add_column("Details", ratio=1, overflow="hidden")
            ...
            if 'rates' in metrics:
                ...
                if 'bytes_recv' in rates:
                    ...
                    table.add_row(
                        "Download",
                        create_responsive_progress_bar(recv_percent, avail_width, custom_color="green"),
                ...
                if 'bytes_sent' in rates:
                    ...
                    table.add_row(
                        "Upload",
                        create_responsive_progress_bar(send_percent, avail_width, custom_color="blue"),
                        f"{format_bytes(send_rate)}/s | Peak: {format_bytes(metrics['peak']['upload'])}/s"
                    )
                
                # Today's usage statistics in more responsive format
                if 'today' in metrics:
                    today = metrics['today']
                    table.add_row(
                        "Today",
            ...
            # Connection statistics - simplified layout
            conn_text = f"Total: {connection_stats['total']}"
            
            # Build compact details string with intelligent spacing
            details = []
            ...
            # Compact TCP/UDP counts for better space usage
            tcp_count = connection_stats['tcp'] + connection_stats['tcp6']
            udp_count = connection_stats['udp'] + connection_stats['udp6']
            
            if tcp_count > 0:
                details.append(f"TCP: {tcp_count}")
            if udp_count > 0:
                details.append(f"UDP: {udp_count}")
            
            # Add connection info in responsive layout
            table.add_row(
                "Connections",
                conn_text,
            ...
            # Ping results if available - compact layout
            if ping_results:
                ping_text = []
                
                # Format and colorize ping results
            ...
            # DNS servers if available - compact layout
            ...
            # Show interfaces - responsive layout
            if interfaces:
                # Find active interfaces (up with IP addresses)
                active_interfaces = []
                ...
                if active_interfaces:
                    ...
                    # Group interfaces for compact display - no more than 2 rows of interfaces
                    for idx, (name, info) in enumerate(active_interfaces[:2]):
                        ipv4 = info['ipv4'][0] if info['ipv4'] else ""  # Show primary IP
                        
                        # Format speed info
                        speed = f"Speed: {info['speed']} Mbps" if info['speed'] else "Speed: Auto"
                        
                        # Add wireless indicator if applicable
                        wireless = " | WiFi" if info.get('wireless', False) else ""
            ...
            # Show error counts if any errors exist - compact layout
            if any(metrics.get(key, 0) > 0 for key in ['error_in', 'error_out', 'drop_in', 'drop_out']):
                table.add_row("", "", "")  # Empty row as separator
                
                # Format error counts
                error_in = metrics.get('error_in', 0)
                error_out = metrics.get('error_out', 0)
                drop_in = metrics.get('drop_in', 0)
                drop_out = metrics.get('drop_out', 0)
                
                # Add error row with neat alignment
                table.add_row(
                    "Errors",
                    f"In: {error_in} | Out: {error_out}",
                    f"Drops - In: {drop_in} | Out: {drop_out}"
                )
...
class DiskMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            table.add_column("Disk", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Usage", ratio=1, overflow="hidden")
            table.add_column("Details", style="bright_blue", ratio=1, overflow="hidden")
            ...
            if 'rates' in io_metrics:
                ...
                # Read/Write rates in responsive layout
                table.add_row(
                    "Disk I/O",
                ...
                # Operations per second in responsive layout
                table.add_row(
                    "Operations",
                ...
                # Busy time if available - using responsive progress bar
                if 'busy_percent' in io_metrics:
                    table.add_row(
                        "Busy",
                        create_responsive_progress_bar(io_metrics['busy_percent'], avail_width),
                        f"{io_metrics['busy_percent']:.1f}% Utilized"
                    )
            
            # Show partitions with responsive progress bars
            for part in partitions:
                name = os.path.basename(part['mountpoint']) or part['mountpoint']
                
                # Create usage bar using responsive progress bar
                usage_bar = create_responsive_progress_bar(part['percent'], avail_width)
                
                # Create details string with proper spacing
                details = [
                ...
                if 'scheduler' in part:
                    # Shorten scheduler name if needed
                    scheduler = part['scheduler']
                    if len(scheduler) > 10:  # Truncate if too long
                        scheduler = scheduler[:8] + "..."
                    details.append(f"Scheduler: {scheduler}")
                
                # Format details with nice spacing
                details_text = " | ".join(details)
                
                # Truncate if too long for display
                if len(details_text) > self.console_width // 3:
                    details_text = details_text[:(self.console_width // 3) - 3] + "..."
...
class ServiceMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            table.add_column("Service", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Status", ratio=1, overflow="hidden")
            table.add_column("Details", style="bright_blue", ratio=1, overflow="hidden")
            ...
            # Calculate available width for status bars
            avail_width = max(20, (self.console_width // 2) - 30)
            ...
            # Show individual services with responsive layout
            for service in sorted_services:
                ...
                # Create status bar - using responsive design without percentage
                status_bar = create_responsive_progress_bar(
                    100 if service['state'] == 'active' else 0, 
                    avail_width,
                ...
                # Add description with intelligent truncation 
                if service['description']:
                    # Calculate available space for description
                    avail_desc_space = max(10, (self.console_width // 3) - 30)
                    desc = service['description']
                    if len(desc) > avail_desc_space:
                        desc = desc[:avail_desc_space-3] + "..."
                    details.append(desc)
                
                # Format name with smart truncation
                service_name = service['name']
                if len(service_name) > col_width:
                    service_name = service_name[:col_width-3] + "..."
...
class GPUMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            table.add_column("GPU", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Usage", ratio=1, overflow="hidden")
            table.add_column("Details", style="bright_blue", ratio=1, overflow="hidden")
            ...
            # Usage row with power info - using responsive progress bar
            power_text = ""
            if metrics.get('power_draw', 0) > 0:
                power_text = f" | {metrics['power_draw']:.1f}W"
            table.add_row(
                "Usage",
                create_responsive_progress_bar(metrics['usage'], avail_width),
                f"{metrics['usage']:.1f}%{power_text}"
            )
            
            # Temperature row with fan speed - using responsive progress bar
            ...
            table.add_row(
                "Temperature",
                create_responsive_progress_bar(metrics['temp'], avail_width, custom_color=temp_color),
                temp_text
            )
            
            # Memory usage row - using responsive progress bar
            table.add_row(
                "Memory",
                create_responsive_progress_bar(metrics['memory_percent'], avail_width),
...
class FirewallMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            table.add_column("Firewall", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Status", ratio=1, overflow="hidden")
            table.add_column("Details", style="bright_blue", ratio=1, overflow="hidden")
            ...
            # Display block rate using responsive progress bar
            # Scale to make it more visible - 10 blocks/sec = 100%
            block_percent = min(100, blocked_rate * 10)
            table.add_row(
                "Blocked",
                create_responsive_progress_bar(block_percent, avail_width),
            ...
            # Display recent rules - limited to terminal size
            max_rules_to_show = min(3, max(1, self.console_width // 50))  # Adjust based on terminal width
            
            for i, rule in enumerate(rules[:max_rules_to_show]):  # Show limited rules
                target_color = {
                    'ACCEPT': 'green',
                    'DROP': 'red',
                    'REJECT': 'red'
                ...
                # Format status and protocol with responsive spacing
                status = Text(rule['target'].upper(), target_color)
                
                # Truncate source/destination if needed
                source = rule['source']
                dest = rule['destination']
                
                if len(source) + len(dest) > self.console_width // 4:
                    if len(source) > 15:
                        source = source[:12] + "..."
                    if len(dest) > 15:
                        dest = dest[:12] + "..."
                ...
                table.add_row(
                    rule['chain'][:col_width],
                    status,
                    f"{rule['protocol']} {source} → {dest}"
                )
...
class SensorMonitor(BaseMonitor):
    ...
    def render(self):
        ...
        try:
            ...
            table.add_column("Sensor", style="cyan", width=col_width, no_wrap=True, overflow="hidden")
            table.add_column("Usage", ratio=1, overflow="hidden")
            table.add_column("Details", style="bright_blue", ratio=1, overflow="hidden")
            ...
            # Show temperatures with responsive layout
            for temp in sensor_data['temperatures']:
                ...
                # Create details text with smart spacing
                ...
                # Format name with proper truncation 
                name = temp['name']
                if len(name) > col_width:
                    name = name[:col_width-3] + "..."
                
                # Use responsive progress bar without percentage text
                table.add_row(
                    name,
                    create_responsive_progress_bar(min(100, temp_percent), avail_width, 
                                                 show_percentage=False, custom_color=status_color),
                    " | ".join(details)
                )
            ...
            # Show fan speeds with responsive layout
            if sensor_data['fans']:
            ...
            if self.history['temps']:
                ...
                # Create trend bar with responsive design
                table.add_row(
                    "Temp Trend",
                    create_responsive_progress_bar(avg_temp, avail_width, show_percentage=False),
                    f"Min: {min_temp:.1f}°C | Avg: {avg_temp:.1f}°C | Max: {max_temp:.1f}°C"
                )
            
            # Add battery info under sensors with responsive layout
            ...
            if battery_info['available']:
                ...
                # Add battery charge row with responsive progress bar
                table.add_row(
                    "Charge",
                    create_responsive_progress_bar(battery_info['percent'], avail_width, custom_color=status_color),
...
class ProcessMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        """Render a panel showing processes sorted by CPU or Memory with responsive layout."""
        try:
            # Update dimensions for responsiveness
            self.update_dimensions()
            ...
            avail_width = self.console_width
            
            pid_width = max(5, min(7, avail_width // 20))  # PID column 
            name_width = max(20, min(25, avail_width // 5))  # Process name - scale with terminal width
            cpu_width = max(5, min(7, avail_width // 20))  # CPU percentage
            mem_width = max(8, min(10, avail_width // 14))  # Memory 
            status_width = max(8, min(10, avail_width // 14))  # Status
            
            table.add_column("PID", style="cyan", width=pid_width, no_wrap=True, overflow="hidden")
            table.add_column("Name", style="bright_blue", width=name_width, no_wrap=True, overflow="hidden")
            table.add_column("CPU%", justify="right", width=cpu_width, no_wrap=True, overflow="hidden")
            table.add_column("Memory", justify="right", width=mem_width, no_wrap=True, overflow="hidden")
            table.add_column("Status", width=status_width, no_wrap=True, overflow="hidden")
            ...
            # Calculate how many processes to show based on available height
            # Default to config, but adjust if terminal is small
            processes_to_show = min(
                self.processes_limit,
                max(5, self.processes_limit) # Reasonable minimum
            )
            
            # Show the processes
            if processes:
                for i, proc in enumerate(processes[:processes_to_show]):
                    # CPU column
                    ...
                    # Format memory text to fit available space
                    mem_text = f"{format_bytes(mem_bytes)} ({mem_percent:.1f}%)"
                    if len(mem_text) > mem_width:
                        mem_text = f"{format_bytes(mem_bytes)}"
                    ...
                    # Process name with truncation if needed
                    proc_name = proc.get('name', '')[:name_width]
            ...
            return Panel(
                table,
                title=f"Process Monitor (Top {processes_to_show}, Sort: {sort_info}) - Press C=CPU, M=Mem",
                border_style="green",
                padding=(0,0)
            )
        except Exception as e:
...
class AlertMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        ...
        try:
            ...
            time_width = max(6, min(8, avail_width // 15))  # Time column
            category_width = max(8, min(12, avail_width // 10))  # Category column 
            
            table.add_column("Time", style="cyan", width=time_width, no_wrap=True, overflow="hidden")
            table.add_column("Category", style="bright_blue", width=category_width, no_wrap=True, overflow="hidden")
            table.add_column("Message", ratio=1, overflow="hidden")
            ...
            # Calculate number of alerts to show based on available space
            alerts_to_show = min(len(recent_alerts), max(3, self.console_width // 30))
            
            # Show alerts from newest to oldest, with most important ones prioritized
            sorted_alerts = sorted(
                recent_alerts,
                key=lambda x: (x['level'] != 'critical', x['timestamp']),
                reverse=True
            )[:alerts_to_show]
            ...
            for alert in sorted_alerts:
                ...
                # Format category with proper truncation
                category = alert['category']
                if len(category) > category_width:
                    category = category[:category_width-3] + "..."
                
                # Message with intelligent truncation based on width
                message = alert['message']
                max_msg_len = max(30, avail_width // 2)
...
class SystemMonitorApp(App):
    ...
    # Use proper Textual CSS with more responsive layout
    CSS = """
    Screen {
        layout: grid;
        grid-size: 2;
    ...
    def __init__(self):
        try:
            ...
            # Create monitor instances with better error handling
            logger.info("Creating monitor instances...")
            self.monitors = {}
            
            # Safe function to create each monitor
            def safe_create_monitor(name, monitor_class):
                try:
    ...
    def on_key(self, event) -> None:
        ...
        try:
            if event.key == "ctrl+c":
                ...
                # Force refresh all monitors on 'r' key
    ...
    def compose(self) -> ComposeResult:
        """Compose the UI with better error handling"""
        try:
            # Header info
            ...
            # Yield with better error handling
            try:
            ...
            # Create footer
            try:
                footer_text = "Press C=CPU sort | M=Memory sort | R=Refresh | Ctrl+C=Exit"
    ...
    def on_mount(self) -> None:
        """Set up monitor layout on mount with better error handling."""
        try:
            # Try to set up the initial monitor layout
            self.refresh_monitors()
    ...
    def refresh_monitors(self) -> None:
        """Refresh the monitor layout with better error handling."""
        try:
            ...
            try:
                ...
                logger.error(f"Failed to remove children: {e}")
    ...
    def _update_header(self) -> None:
        """Update the header with better error handling."""
        try:
            uptime = datetime.now() - self.start_time
            uptime_str = f"{uptime.seconds // 3600}h {(uptime.seconds % 3600) // 60}m {uptime.seconds % 60}s"
