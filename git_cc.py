...
def do_gitignore():
    ...
    # ── Presets ──
    print("\n  These are ready-made ignore lists based on your project type.")
    print("  Pick the ones that match — you can pick multiple.\n")
    print("   1 = Python  → ignores compiled files, virtual envs, build output")
    print("   2 = Node.js → ignores node_modules folder, build output, logs")
    print("   3 = C/C++   → ignores compiled binaries, .o files, build folders")
    print("   4 = Java    → ignores .class files, .jar files, build folders")
    print("   5 = General → ignores editor junk (.vscode/.idea), OS files, logs")
    print("   0 = None of these, I'll add my own below")
...
def do_delete():
    section("🗑️  Delete files or folders from GitHub")

    _, _, code = run("git log --oneline -1", capture=True)
    if code != 0:
        print("  ⏭️  No commits yet — nothing to delete from GitHub. Skipping.")
        return

    # Show what is currently tracked in git
    tracked, _, _ = run("git ls-files", capture=True)
    if tracked:
        print("\n  These files are currently tracked in your repo:")
    ...
    while True:
        choice = choose("What do you want to delete?", [
            "A single file",
            "An entire folder",
        ...
        if choice == "a":
            print("\n  Type the file name exactly as shown in the list above.")
            target = input("\n  File to delete: ").strip()
            is_folder = False

            # Check it's tracked in git (even if not local)
            tracked_list = tracked.splitlines()
            if target not in tracked_list:
                print(f"  ❌ '{target}' not found in git. Check the name matches the list above exactly.")
                continue

            cmd_cached = f'git rm --cached "{target}"'
            cmd_local  = f'git rm "{target}"'

        else:
            print("\n  Type the folder name exactly as shown in the list above (without trailing slash).")
            print("  Example: if you see  old/file.txt  in the list, type:  old")
            target = input("\n  Folder to delete: ").strip().rstrip("/")
            is_folder = True

            # Check at least one tracked file starts with that folder
            tracked_list = tracked.splitlines()
            matches = [f for f in tracked_list if f.startswith(target + "/") or f == target]
            if not matches:
                print(f"  ❌ No tracked files found in folder '{target}'. Check the name matches the list above.")
                continue

            print(f"  Found {len(matches)} file(s) in '{target}/':")
            for m in matches:
                print(f"   {m}")

            cmd_cached = f'git rm -r --cached "{target}"'
            cmd_local  = f'git rm -r "{target}"'

        also_local = choose(f"Also delete '{target}' from your LOCAL machine too?", [
            "Yes, delete locally too",
            "No, only remove from GitHub, keep it on my computer"
        ])

        cmd = cmd_local if also_local == "yes," else cmd_cached

        _, err, code = run(cmd, capture=True)
        if code != 0:
            print(f"  ❌ Could not remove '{target}': {err}")
        else:
            _, err2, code2 = run(f'git commit -m "remove {target}"', capture=True)
            if code2 == 0:
                print(f"  ✅ '{target}' removed and committed! Will be deleted from GitHub on next push.")
                # Refresh tracked list
                tracked, _, _ = run("git ls-files", capture=True)
            else:
                print(f"  ❌ Commit failed: {err2}")

        more = choose("Delete anything else?", ["Yes, delete more", "No, done deleting"])
        if more == "no,":
            return
