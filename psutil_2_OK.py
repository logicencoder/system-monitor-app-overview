...
class NetworkMonitor(BaseMonitor):
    ...
    def render(self) -> Panel:
        """Render enhanced network information panel."""
        try:
            table = Table(box=None, expand=True, padding=(0,1))
            ...
            if 'rates' in metrics:
                ...
                if 'bytes_recv' in rates:
                    ...
                    table.add_row(
                        ...
                        f"Total: {format_bytes(metrics['bytes_recv'])} | Peak: {format_bytes(metrics['peak']['download'])}/s"
                    )
                ...
                if 'bytes_sent' in rates:
                    ...
                    table.add_row(
                        ...
                        f"Total: {format_bytes(metrics['bytes_sent'])} | Peak: {format_bytes(metrics['peak']['upload'])}/s"
                    )
                ...
                if 'today' in metrics:
                    ...
                    table.add_row(
                        "Today's Data",
                        f"↓ {format_bytes(today['download'])}",
                        f"↑ {format_bytes(today['upload'])} | Total: {format_bytes(today['total'])}"
                    )
            ...
            if connection_stats['tcp'] > 0 or connection_stats['tcp6'] > 0:
                details.append(f"TCP: {connection_stats['tcp']+connection_stats['tcp6']}")
            if connection_stats['udp'] > 0 or connection_stats['udp6'] > 0:
            ...
            for name, info in interfaces.items():
                ...
                if info.get('wireless'):
                    signal_strength = info.get('signal', 0)
                    # Convert to percentage or quality indicator
                    if signal_strength:
                        wireless_info = f" | Signal: {signal_strength:.0f}dBm"
                
                table.add_row(
                    name[:12],
                    f"Speed: {speed_text}{duplex_text}{wireless_info}",
                    " | ".join(details)
                )
...
class SensorMonitor(BaseMonitor):
    ...
    def _create_bar_without_percentage(self, percentage, width=40, color=None):
        ...
        try:
            ...
            # Create the bar with the specified color - NO PERCENTAGE TEXT
            bar = Text('■' * filled, color) + Text('·' * remainder, "bright_black")
            return bar  # No percentage text added
        except Exception as e:
            logger.error(f"Error creating progress bar: {e}")
            return Text("Error", "red")
    ...
    def render(self):
        ...
        try:
            ...
            for temp in sensor_data['temperatures']:
                ...
                # Use our custom progress bar function WITHOUT percentage text
                table.add_row(
                    temp['name'][:12],
            ...
            if self.history['temps']:
                ...
                table.add_row(
                    "Temp Trend",
                    self._create_bar_without_percentage(avg_temp),
